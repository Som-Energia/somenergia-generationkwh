...
class Migrator:
            ...
            for partner_id in sorted(self.movementsByPartner):
                ...
                for movement in movements:
                    ...
                    if 'solution' in movement:
                        output.write("\t\t{name}\t{type}\t{orderline_id: 10d}\t{ip}\n".format(**movement.solution))
        ...
        with Path('unsolved.yaml').open('w', encoding='utf8') as output:
            ...
            for partner_id in sorted(self.movementsByPartner):
                ...
                for movement in movements:
                    ...
                    output.write("#    {id}: uknown # {date_created} ({partner_name}) {amount} {name}\n".format(**movement))
    ...
    def matchPaymentOrders(self):
        ...
        for data in order2MovementMatches:
            ...
            orderLines = getOrderLines(self.cr, data.order_id)
            moveLines = getMoveLines(self.cr, data.move_id)
